...
def llm_completion(model: str, messages: list[dict], api_key: str | None = None, api_base: str | None = None, temperature: float = 0.0, max_tokens: int = 1024, num_retries: int = 2, timeout: int = 60):
...
def safe_parse_llm_json(text: str) -> dict:
    ...
    if parsed is None:
        return {"error": f"Failed to parse LLM JSON response"}
